...
import { DEFAULT_PM_SCHEDULES } from '../src/modules/settings/settings.defaults';
import { ensureSettingsSeeded } from '../src/modules/settings/settings.service';

const prisma = new PrismaClient();
const hash = (p: string) => bcrypt.hash(p, 10);
const daysFromNow = (n: number) => dayjs().add(n, 'day').toDate();
...
async function main() {
  ...
  for (const [vt, cfg] of Object.entries(DEFAULT_PM_SCHEDULES)) {
    await prisma.pmSchedule.upsert({
      where: { vehicleType: vt as VehicleType },
      create: { vehicleType: vt as VehicleType, kmInterval: cfg.km, timeIntervalDays: cfg.days },
  ...
  const users: { email: string; role: Role; name: string; password: string }[] = [
  ...
  // --- Vendors ---
  const vendorData = [
    { type: 'workshop' as const, name: 'Al Habtoor Motors Workshop', phone: '+97143334444' },
    { type: 'tyre_supplier' as const, name: 'ZAFCO Tyres', phone: '+97148889999' },
    { type: 'insurance' as const, name: 'Oman Insurance Company', phone: '+97142330000' },
    { type: 'fuel_supplier' as const, name: 'FuelBuddy UAE', phone: '+97145556666' },
    { type: 'lessor' as const, name: 'Diamond Lease', phone: '+97143001000' },
  ];
  const vendors = [];
  ...
  const stores = [];
  for (const s of storeData) stores.push(await prisma.store.create({ data: s }));

  // --- Vehicles (~10) ---
  const makes: [string, string, VehicleType, string][] = [
  ...
  for (let i = 0; i < makes.length; i++) {
    ...
    const cfg = DEFAULT_PM_SCHEDULES[type];
